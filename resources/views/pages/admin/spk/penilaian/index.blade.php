    ...
    <div class="bg-white shadow-xl rounded-lg p-6">
        <header class="mb-6 border-b pb-4 flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('alternatif.index', $keputusan->id_keputusan) }}" 
                ...
                <a href="{{ route('admin.spk.calculate.proses', $keputusan->id_keputusan) }}" 
                    ...
                    <i class="fas fa-calculator mr-1"></i> Lanjut ke Perhitungan
                </a>
            </div>
        </header>
        ...
        @if ($kriteriaList->isNotEmpty() && $alternatifData->isNotEmpty())
            <form action="{{ route('penilaian.update', $keputusan->id_keputusan) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="overflow-x-auto border border-gray-200 rounded-lg shadow-md mb-6">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="w-1/4 px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider border-r">
                                    Alternatif / Kriteria
                                </th>
                                {{-- Header Kriteria --}}
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            @foreach($alternatifData as $alternatif)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-semibold text-gray-900 border-r bg-gray-50">
                                    ...
                                    @foreach($kriteriaList as $kriteria)
                                        <td class="px-4 py-2 text-center">
                                            ...
                                            @if($isSubkriteria)
                                                ...
                                                <select name="{{ $inputName }}" class="block w-full py-2 px-1 text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div class="flex justify-end mt-4">
                    <button type="submit" class="inline-flex items-center px-6 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150">
            ...
            <div class="mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($kriteriaList as $kriteria)
                        <div class="p-4 border rounded-lg shadow-sm {{ $kriteria->subKriteria->count() > 0 ? 'bg-indigo-50 border-indigo-300' : 'bg-gray-50 border-gray-300' }}">
                            ...
                            @if ($kriteria->subKriteria->count() > 0)
                                ...
                                <ul class="list-disc list-inside mt-1 text-xs text-gray-700">
                                    @foreach($kriteria->subKriteria as $sub)
                                        <li>{{ $sub->nama_subkriteria }}: **{{ number_format($sub->nilai_konversi, 2) }}**</li>
                                    @endforeach
                                </ul>
                            @else
                                <p class="mt-2 text-sm font-medium text-gray-700">Mode: Input Langsung (Angka)</p>
                                <p class="text-xs text-gray-600 italic">Nilai diinput sebagai angka mentah (misalnya: skor, jumlah, atau rating).</p>
                            @endif
                        </div>
                    @endforeach
                </div>
