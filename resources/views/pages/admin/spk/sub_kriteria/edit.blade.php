    ...
    <div class="bg-white shadow-xl rounded-lg p-8">
        <header class="mb-8 border-b pb-4">
            ...
            <p class="text-sm text-gray-600">
                Mengedit Sub Kriteria untuk: <span class="font-semibold text-indigo-600">{{ $kriteria->kode_kriteria }} - {{ $kriteria->nama_kriteria }}</span>
            </p>
        </header>
        ...
        <form action="{{ route('kriteria.subkriteria.update', [$keputusan->id_keputusan, $kriteria->id_kriteria, $subkriteria->id_subkriteria]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    <label for="nilai_konversi" class="block text-sm font-medium text-gray-700 mb-1">Nilai Konversi</label>
                    <input type="number" name="nilai_konversi" id="nilai_konversi" 
                           value="{{ old('nilai_konversi', number_format($subkriteria->nilai_konversi, 4, '.', '')) }}"
                           step="any" min="0"
                           class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm
                           @error('nilai_konversi') border-red-500 @enderror"
            ...
            <div class="mt-8 flex justify-end space-x-3">
                <a href="{{ route('kriteria.subkriteria.index', [$keputusan->id_keputusan, $kriteria->id_kriteria]) }}" 
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-edit mr-2"></i> Perbarui Sub Kriteria
                </button>
            </div>
        </form>
    </div>
