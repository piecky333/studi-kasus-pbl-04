...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="md:flex md:items-center md:justify-between mb-8">
        ...
        <div class="mt-4 flex md:mt-0 md:ml-4">
            <a href="{{ route('pengurus.prestasi.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Kembali
            </a>
        </div>
    </div>
    ...
    <div class="bg-white shadow-[10px_10px_15px_-3px_rgba(0,0,0,0.1)] overflow-hidden sm:rounded-lg border border-gray-200 max-w-3xl mx-auto">
        ...
        <div class="px-4 py-5 sm:p-6">
            ...
            <form action="{{ route('pengurus.prestasi.update', $prestasi) }}" method="POST">
                @csrf
                @method('PUT')
...
document.getElementById('nim').addEventListener('keyup', function() {
    ...
    if (query.length > 1) {
        fetch(`/pengurus/prestasi/cari-mahasiswa?nim=${query}`)
            .then(res => res.json())
            .then(data => {
                if (data.length > 0) {
