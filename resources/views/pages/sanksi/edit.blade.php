...
<!-- Page Heading -->
<h1 class="h3 mb-4 text-gray-800">Edit Data Sanksi</h1>

<div class="card shadow mb-4">
    <!-- Card Header -->
    <div class="card-header py-3 bg-primary">
        <h6 class="m-0 font-weight-bold text-white">Formulir Edit Sanksi</h6>
    </div>
        ...
        <form action="{{ route('admin.sanksi.update', $sanksi->id_sanksi) }}" method="POST">
                ...
                <select name="id_mahasiswa" id="id_mahasiswa" 
                    ...
                    @foreach($mahasiswa as $mhs)
                        <option value="{{ $mhs->id_mahasiswa }}" 
                            {{ (old('id_mahasiswa', $sanksi->id_mahasiswa) == $mhs->id_mahasiswa) ? 'selected' : '' }}>
                            {{ $mhs->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="date" id="tanggal_sanksi" name="tanggal_sanksi" 
                       ...
                       value="{{ old('tanggal_sanksi', $sanksi->tanggal_sanksi) }}">
